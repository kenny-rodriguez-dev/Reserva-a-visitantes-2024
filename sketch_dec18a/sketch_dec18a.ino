...
const char* ssid = "XTRIM_RODRIGUEZ_2.4G";
const char* password = "Alexrm2024@";
/* ======================================== */
...
<html>
  ...
  <body>
    ...
    <h3 style="padding-top: 30px;">Escaner QR</h3>
...
/* ____________________________ Index handler function to be called during GET or uri request */
...
/* ____________________________ stream handler function to be called during GET or uri request. */
...
/* ____________________________ cmd handler function to be called during GET or uri request. */
...
/* ____________________________ qrcoderslt handler function to be called during GET or uri request. */
...
/* ____________________________ Subroutine for starting the web server / startCameraServer. */
...
/* ____________________________ create "QRCodeReader_Task" using the xTaskCreatePinnedToCore() function */
...
/* ____________________________ QRCodeReader() */
...
void processQRCodeData(const String& qrCode) {
  // Realizar una solicitud HTTP a Django
  HTTPClient http;
  http.begin("http://192.168.1.9:8000/procesar_qr/");
  http.addHeader("Content-Type", "application/x-www-form-urlencoded");

  String postData = "qr_code=" + qrCode;
  ...
  if (httpCode > 0) {
    ...
    if (jsonError) {
      ...
      if (success) {
        ...
        // Encender el LED azul durante 3 segundos
...
/* ____________________________ Subroutine to execute commands based on the results of reading the QR Code. */
void cmd_execution() {
  ...
  processQRCodeData(QRCodeResult);
}
/* ____________________________ */

/* ____________________________ String function to process the data command */
